...
/**
 * @author wpq
 * @function 日期转换工具类<br> 包含：<br> 1.Date、LocalDate、LocalDateTime、LocalTime、Instant、ZonedDateTime、YearMonth、Timestamp、时间戳和TemporalAccessor等互相转换<br>
 * 2.天、小时、分钟、秒和毫秒等时间单位相互转换，支持小单位到大单位的精确转换比如，minuteToHourPrecise(long num) 90分钟转换为小时，为1.5小时。<br>
 * 3.转换ZonedDateTime的同时支持转换为指定时区，比如toZonedDateTime(Date date, String zoneId)
 * ,toZonedDateTimeAndTransformZone(LocalDateTime localDateTime, String targetZoneId)。<br>
...
public class DateTimeConverterUtil {
  ...
  public static Date toDate(String date) throws ParseException {
    ...
    Date endDateTime = simpleDateFormat.parse(date);
    return endDateTime;
  }
  ...
  /**
   * YearMonth转Date 注意dayOfMonth范围：1到31之间，最大值根据月份确定特殊情况，如2月闰年29，非闰年28
  ...
  /**
   * YearMonth转LocalDate 注意dayOfMonth范围：1到31之间，最大值根据月份确定特殊情况，如2月闰年29，非闰年28
  ...
  /**
   * LocalDateTime转ZonedDateTime，时区为zoneId对应时区<br> 注意，需要保证localDateTime和zoneId是对应的，不然会出现错误<br>
   ...
   * @return ZonedDateTime
   */
  public static ZonedDateTime toZonedDateTimeAndTransformZone(LocalDateTime localDateTime,
      String targetZoneId) {
    Objects.requireNonNull(localDateTime, "localDateTime");
    Objects.requireNonNull(targetZoneId, "targetZoneId");
    return DateTimeCalculatorUtil
        .transform(localDateTime.atZone(ZoneId.systemDefault()), targetZoneId);
  }
   ...
   * @return 返回结果
   */
  public static BigDecimal unitConversionPrecise(long sourceDuration, TimeUnit sourceUnit,
      TimeUnit targetUnit, int scale, RoundingMode roundingMode) {
    return BigDecimal.valueOf(sourceDuration)
        .multiply(BigDecimal.valueOf(sourceUnit.toNanos(1)))
        .divide(BigDecimal.valueOf(targetUnit.toNanos(1)), scale, roundingMode);
  }
   ...
   * @return 返回结果
   */
  public static String unitConversionPreciseString(long sourceDuration, TimeUnit sourceUnit,
      TimeUnit targetUnit, int scale, RoundingMode roundingMode) {
    return unitConversionPrecise(sourceDuration, sourceUnit, targetUnit, scale, roundingMode)
  ...
  public static BigDecimal millisecondToSecondPreciseThree(long num) {
    return unitConversionPrecise(num, TimeUnit.MILLISECONDS, TimeUnit.SECONDS, 3,
        RoundingMode.DOWN);
  }
