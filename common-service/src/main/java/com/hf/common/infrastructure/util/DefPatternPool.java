...
public final class DefPatternPool {
  ...
  /**
   * 密码强度校验，最少6位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符 密码至少包含大小写字母、数字、字符，且不少于6位
   */
  public final static Pattern SECURITY_PASSWORD = Pattern
      .compile("^\\S*(?=\\S{6,})(?=\\S*\\d)(?=\\S*[A-Z])(?=\\S*[a-z])(?=\\S*[!@#$%^&*?.])\\S*$");

  /**
   * 占位符  /\$\{(\w+)\}/g ${xxxx}
   */
  public final static Pattern PLACEHOLDER = Pattern
      .compile("\\$\\{([\\w\\.\\-\\/\\+\\$\\#\\@\\!\\^\\&\\(\\)]+)\\}");
