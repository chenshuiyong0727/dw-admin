...
import com.hf.common.infrastructure.resp.ValidatorMsg;
import com.hf.common.infrastructure.validator.ValidatorLenMax;
import com.hf.common.infrastructure.validator.ValidatorLenMin;
...
public final class ValidatorUtil {
  ...
  public static void verify(Object obj) {
    ...
    for (Field field : fields) {
      // 获得 统一验证 注解 （起码冲突了）
      com.hf.common.infrastructure.validator.Validator validator =
          field.getAnnotation(com.hf.common.infrastructure.validator.Validator.class);
      if (validator != null) {
...
//        int strLength = value.getBytes(StandardCharsets.UTF_8).length;
        ...
        if (strLength > maxLength) {
          throw new ServiceException(ValidatorMsg.EXCEPTION_IS_MAX.getCode(),
              ValidatorMsg.EXCEPTION_IS_MAX.getMessage1(fieldName) + " '" + maxLength + "'");
        }
      }
    } catch (ServiceException e) {
      throw e;
...
//        int strLength = value.getBytes(StandardCharsets.UTF_8).length;
        ...
        if (strLength < minLength) {
          throw new ServiceException(ValidatorMsg.EXCEPTION_IS_MIN.getCode(),
              ValidatorMsg.EXCEPTION_IS_MIN.getMessage1(fieldName) + " '" + minLength + "'");
        }
      }
    } catch (ServiceException e) {
      throw e;
